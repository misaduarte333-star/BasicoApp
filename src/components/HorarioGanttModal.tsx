...
import type { Profesional, Sucursal, DiasSemana } from '@/lib/types'
...
interface HorarioGanttModalProps {
    isOpen: boolean
    onClose: () => void
    profesionales: (Profesional & { sucursal?: Partial<Sucursal> })[]
    sucursal?: Partial<Sucursal> | null
}

/**
 * Componente Modal que muestra un cronograma (Diagrama de Gantt) de la disponibilidad
 * y turnos de los profesionales en contraste con el horario de apertura de la sucursal.
 */
export function HorarioGanttModal({ isOpen, onClose, profesionales, sucursal }: HorarioGanttModalProps) {
    const { getLabel, businessName } = useBusinessLabels()
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            <div className="bg-background rounded-2xl shadow-2xl border border-border-muted w-full max-w-6xl max-h-[90vh] flex flex-col">
                ...
                <div className="p-6 border-b border-border-muted flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-bold text-foreground">Horarios de {businessName}</h2>
                ...
                <div className="flex-1 overflow-auto p-6">
                    <div className="min-w-[800px]">
                        ...
                        <div className="grid grid-cols-[200px_1fr] mb-4">
                            <div className="text-muted-foreground/70 text-xs uppercase font-bold tracking-wider pt-2">{getLabel('professional', 'singular')} / Hora</div>
                        ...
                        <div className="space-y-4">
                            {profesionales.map(profesional => {
                                const workSchedule = profesional.horario_laboral[selectedDay]

                                return (
                                    <div key={profesional.id} className="grid grid-cols-[200px_1fr] group hover:bg-surface/ rounded-lg transition-colors">
                                        <div className="flex items-center gap-3 py-2 pr-4 pl-2">
                                            ...
                                            <div>
                                                <p className="font-medium text-slate-200 text-sm">{profesional.nombre}</p>
                                                                                                <p className="text-[10px] text-muted-foreground/70">{getLabel('station', 'singular', true)} {profesional.estacion_id}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid relative items-center" style={{ gridTemplateColumns: `repeat(${hours.length}, 1fr)` }}>
                                            ...
                                            {hours.map(hour => {
                                                ...
                                                if (workSchedule && profesional.bloqueo_almuerzo && profesional.bloqueo_almuerzo.inicio && profesional.bloqueo_almuerzo.fin) {
                                                    const startH = parseInt(profesional.bloqueo_almuerzo.inicio.split(':')[0])
                                                    const endH = parseInt(profesional.bloqueo_almuerzo.fin.split(':')[0])
                ...
                <div className="p-4 border-t border-border-muted flex gap-6 text-xs text-muted-foreground">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span>Turno {getLabel('professional', 'singular')}</span>
                    </div>
                    <div className="flex items-center gap-2">
