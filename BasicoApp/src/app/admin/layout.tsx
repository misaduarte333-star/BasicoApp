...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    const { businessName, professionals, services } = useBusinessLabels()
            ...
            >
                <div className="p-4 h-full flex flex-col relative">
                    ...
                    <Link href="/admin" className={`flex items-center gap-3 mb-10 text-foreground hover:opacity-80 transition-opacity ${isCollapsed ? 'justify-center' : ''}`}>
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center shadow-lg shadow-blue-900/20 flex-shrink-0 text-white font-bold">
                            {businessName.charAt(0)}
                        </div>
                        <div className={`transition-all duration-300 overflow-hidden ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}>
                            <h1 className="font-bold text-lg leading-none whitespace-nowrap truncate max-w-[150px]">{businessName}</h1>
                            <p className="text-xs text-muted-foreground mt-1 whitespace-nowrap">Panel Control</p>
                        </div>
                    </Link>
                    ...
                    <nav className="space-y-1 flex-1">
                        ...
                        <NavItem href="/admin/citas" icon="calendar" label="Agenda" active={isActive('citas')} collapsed={isCollapsed} />
                        <NavItem href="/admin/profesionales" icon="users" label={professionals} active={isActive('profesionales')} collapsed={isCollapsed} />
                        <NavItem href="/admin/servicios" icon="scissors" label={services} active={isActive('servicios')} collapsed={isCollapsed} />
                    ...
                    <div className="pt-6 border-t border-slate-700/50 space-y-4">
                        ...
                        <button 
                            onClick={logout}
                            className={`w-full flex items-center ${isCollapsed ? 'justify-center gap-0 px-2' : 'gap-3 px-4'} py-3 rounded-xl transition-all duration-200 text-red-400 hover:text-red-300 hover:bg-red-500/10 group`}
                            title="Cerrar Sesión"
                        >
            ...
            <main className={`flex-1 ${isCollapsed ? 'md:ml-20' : 'md:ml-64'} p-4 md:p-8 min-h-screen transition-all duration-300 flex flex-col`}>
                ...
                <div className="md:hidden flex items-center justify-between mb-6">
                    <div className="flex items-center">
                        ...
                        <span className="ml-4 font-bold text-foreground text-lg">{businessName}</span>
                    </div>
                </div>
