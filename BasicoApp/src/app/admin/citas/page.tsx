...
import type { CitaConRelaciones, EstadoCita, Servicio, Profesional } from '@/lib/types'
...
export default function CitasPage() {
    ...
    const cargarCitas = useCallback(async () => {
        ...
        try {
          ...
          *,
          servicio:servicios(*),
          profesional:barberos(nombre, estacion_id)
        `).eq('sucursal_id', sucursalId)
                    ...
                    citas.map((cita) => (
                        <div key={cita.id} className="glass-card p-4 relative">
                            ...
                            <div className="grid grid-cols-2 gap-2 mb-4 text-sm">
                                ...
                                <div className="bg-surface/ p-2 rounded">
                                    <p className="text-xs text-muted-foreground/70 mb-0.5">{professional}</p>
                                    <p className="text-slate-200 truncate">{cita.profesional?.nombre || 'Sin asignar'}</p>
                                </div>
                            </div>
                        ...
                        <table className="w-full min-w-[800px]">
                            <thead className="bg-surface/">
                                <tr>
                                    ...
                                    <th className="px-3 md:px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase">{professional}</th>
                                    ...
                                    <tr key={cita.id} className="hover:bg-surface/ transition-colors">
                                        ...
                                        <td className="px-3 md:px-6 py-4">
                                            ...
                                            <div className="text-xs text-muted-foreground/70">
                                                {cita.profesional?.nombre || `Sin ${professional.toLowerCase()}`}
                                            </div>
                                        </td>
                                        <td className="px-3 md:px-6 py-4">
                                            <span className={`
    ...
    return [
        {
            ...
            profesional: { id: '1', sucursal_id: '1', nombre: 'Carlos H.', estacion_id: 1, usuario_tablet: '', password_hash: '', horario_laboral: {}, bloqueo_almuerzo: null, activo: true, hora_entrada: null, comision_porcentaje: 0, meta_cortes_mensual: 0, created_at: '' }
        },
        {
            id: '2',
            sucursal_id: '1',
        ...
        },
        {
            ...
            profesional: { id: '2', sucursal_id: '1', nombre: 'Miguel L.', estacion_id: 2, usuario_tablet: '', password_hash: '', horario_laboral: {}, bloqueo_almuerzo: null, activo: true, hora_entrada: null, comision_porcentaje: 0, meta_cortes_mensual: 0, created_at: '' }
        }
    ]
}
...
function CitaModal({
    ...
    const [formData, setFormData] = useState({
        ...
        profesional_id: cita?.barbero_id || '',
    ...
    useEffect(() => {
        ...
        const loadDeps = async () => {
            ...
            // Load Professionals
            const { data: profesionalesData } = await supabase.from('barberos')
                .select('*')
                .eq('activo', true)
                .eq('sucursal_id', sucursalId)
            if (profesionalesData) setProfesionales(profesionalesData)
        }
        loadDeps()
    }, [sucursalId])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const newCita = {
                ...
                barbero_id: formData.profesional_id || null, // Optional profesional
            ...
            <div className="glass-card w-full max-w-lg animate-slide-in max-h-[90vh] overflow-y-auto border border-border">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-muted mb-2">{professional}</label>
                            <select
                                className="input-field"
