...
import type { Profesional, ProfesionalConSucursal, Sucursal } from '@/lib/types'
import { HorarioGanttModal } from '@/components/HorarioGanttModal'

/**
 * Página para la gestión del equipo de profesionales.
 * Permite visualizar el listado de profesionales, ver su diagrama de disponibilidad (Gantt),
...
export default function ProfesionalesPage() {
    ...
    const [profesionales, setProfesionales] = useState<ProfesionalConSucursal[]>([])
    ...
    /**
     * Recupera la lista de profesionales de la base de datos junto con la información
    ...
    const cargarProfesionales = useCallback(async () => {
        ...
        try {
            ...
            if (error) {
                console.warn('Error loading profesionales:', formatError(error))
                // Demo data
                setProfesionales(getDemoProfesionales())
            } else {
    ...
    const handleDelete = async (id: string) => {
        if (!confirm(`¿Estás seguro de eliminar este ${getLabel('professional', 'singular')}?`)) return

        try {
            const res = await fetch(`/api/profesionales?id=${id}`, { method: 'DELETE' })
    ...
    const filteredProfesionales = profesionales.filter(b =>
        ...
        <>
            <div className="mb-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-foreground">{getLabel('professional', 'plural', true)}</h1>
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                        ...
                        <button onClick={handleNew} className="btn-primary flex items-center gap-2">
                            ...
                            Nuevo {getLabel('professional', 'singular', true)}
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="glass-card p-4 mb-6">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-muted-foreground text-sm">{filteredProfesionales.length} {getLabel('professional', 'plural')}</span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:hidden mb-6">
                {loading ? (
                    <div className="p-8 text-center text-muted-foreground">Cargando...</div>
                     ...
                     <div className="glass-card p-6 text-center text-muted-foreground">No se encontraron {getLabel('professional', 'plural')}</div>
                ) : (
                    filteredProfesionales.map(profesional => (
                        <div key={profesional.id} className="glass-card p-4 relative">
                            <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center gap-3">
                                     <div className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-600 to-purple-400 flex items-center justify-center font-bold text-foreground text-lg">
                                        {profesional.estacion_id}
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-foreground">{profesional.nombre}</h3>
                                        <p className="text-xs text-muted-foreground">{profesional.usuario_tablet}</p>
                                    </div>
                                </div>
                                <span className={`status-badge ${profesional.activo ? 'status-in-progress' : 'status-cancelled'}`}>
                            ...
                            <div className="space-y-2 text-sm text-muted mb-4">
                                <div className="flex justify-between">
                                    ...
                                    <span>{getHorarioResumen(profesional.horario_laboral)}</span>
                                </div>
                                {profesional.bloqueo_almuerzo && (
                                     <div className="flex justify-between">
                                        <span className="text-foreground">Comida:</span>
                                        <span>{profesional.bloqueo_almuerzo.inicio} - {profesional.bloqueo_almuerzo.fin}</span>
                                    </div>
                                )}
                            </div>

                            <div className="flex justify-end gap-2 border-t border-slate-700/50 pt-3">
                                <button onClick={() => handleEdit(profesional)} className="p-2 rounded-lg bg-surface-hover text-muted hover:bg-slate-600 transition-colors">
                                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                </button>
                                <button onClick={() => { setEditingProfesional(profesional); setShowScheduleModal(true); }} className="p-2 rounded-lg bg-blue-500/10 text-blue-400">
                                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                </button>
                                <button onClick={() => handleDelete(profesional.id)} className="p-2 rounded-lg bg-red-500/10 text-red-400">
            ...
            <div className="glass-card overflow-hidden hidden md:block">
                {loading ? (
                    ...
                    <div className="p-12 text-center">
                        ...
                        <p className="text-muted-foreground">No se encontraron {getLabel('professional', 'plural')}</p>
                    </div>
                ) : (
                    <table className="w-full">
                        <thead className="bg-surface/">
                            <tr>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">{getLabel('station', 'singular', true)}</th>
                                <th className="px-6 py-4 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider">{getLabel('professional', 'singular', true)}</th>
                        ...
                        <tbody className="divide-y divide-slate-700/50">
                            {filteredProfesionales.map((profesional) => (
                                <tr key={profesional.id} className="hover:bg-surface/ transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="font-medium text-foreground">{profesional.nombre}</div>
                                                <div className="text-sm text-muted-foreground">{getLabel('professional', 'singular', true)}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-muted-foreground">
                                        {profesional.usuario_tablet}
                                    </td>
                                    <td className="px-6 py-4 text-muted-foreground">
                                        {getHorarioResumen(profesional.horario_laboral)}
                                    </td>
                                    <td className="px-6 py-4">
                                        <span className={`status-badge ${profesional.activo ? 'status-in-progress' : 'status-cancelled'}`}>
                                            {profesional.activo ? 'Activo' : 'Inactivo'}
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            <button onClick={() => handleEdit(profesional)} className="p-2 rounded-lg hover:bg-surface-hover text-muted-foreground transition-colors">
                                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                                            </button>
                                            <button onClick={() => { setEditingProfesional(profesional); setShowScheduleModal(true); }} className="p-2 rounded-lg hover:bg-blue-900/20 text-blue-400 transition-colors">
                                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                            </button>
                                            <button onClick={() => handleDelete(profesional.id)} className="p-2 rounded-lg hover:bg-red-900/20 text-red-400 transition-colors">
                                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                            </button>
                                        </div>
                                    </td>
            ...
            {showScheduleModal && editingProfesional && (
                <HorarioModal
                    profesional={editingProfesional}
...
// Demo data
function getDemoProfesionales(): ProfesionalConSucursal[] {
    return [
        {
            id: '1',
            sucursal_id: '1',
...
function ProfesionalModal({
    ...
    const [formData, setFormData] = useState({
        ...
        estacion_id: profesional?.estacion_id?.toString() || '',
        usuario_tablet: profesional?.usuario_tablet || '',
        password: '',
        activo: profesional?.activo ?? true,
        comision_porcentaje: profesional?.comision_porcentaje || 50,
        meta_cortes_mensual: profesional?.meta_cortes_mensual || 100
    })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const data: Record<string, unknown> = {
                ...
                comision_porcentaje: Number(formData.comision_porcentaje),
                meta_cortes_mensual: Number(formData.meta_cortes_mensual),
                horario_laboral: profesional?.horario_laboral || {
                    lunes: { inicio: '09:00', fin: '18:00' },
                    martes: { inicio: '09:00', fin: '18:00' },
                    miercoles: { inicio: '09:00', fin: '18:00' },
                    jueves: { inicio: '09:00', fin: '18:00' },
            ...
            let savedProfesionalId: string | null = profesional?.id || null
            ...
            if (profesional) {
                ...
                const res = await fetch('/api/profesionales', {
                    ...
                    body: JSON.stringify({ id: profesional.id, ...data })
                })
                const result = await res.json()
                ...
                savedProfesionalId = result.data?.id || null
            }

            // Set password via API (uses bcrypt on the server)
            if (formData.password && savedProfesionalId) {
                const res = await fetch('/api/auth/reset-password', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        table: 'barberos', // Mantener nombre de tabla DB
                        userId: savedProfesionalId,
            ...
            <div className="glass-card w-full max-w-lg animate-slide-in border border-border">
                <div className="flex items-center justify-between p-6 border-b border-border">
                    <h2 className="text-xl font-bold text-foreground">
                        {profesional ? `Editar ${getLabel('professional', 'singular', true)}` : `Nuevo ${getLabel('professional', 'singular', true)}`}
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-muted mb-2">
                                {profesional ? 'Nueva Contraseña (opcional)' : 'Contraseña'}
                            </label>
                            <input
                                type="password"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-muted mb-2">Comisión (%)</label>
                            <input
                                type="number"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-muted mb-2">Meta de Cortes Mensual</label>
                            <input
                                type="number"
                                value={formData.meta_cortes_mensual}
                                onChange={(e) => setFormData({ ...formData, meta_cortes_mensual: Number(e.target.value) })}
                                className="input-field"
                                placeholder="100"
                            />
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <label htmlFor="activo" className="text-sm text-muted">
                            {getLabel('professional', 'singular', true)} activo
                        </label>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-border">
                        ...
                        <button type="submit" disabled={loading} className="btn-primary flex items-center gap-2">
                            ...
                            {profesional ? 'Guardar Cambios' : `Crear ${getLabel('professional', 'singular', true)}`}
                        </button>
                    </div>
                </form>
            </div>
...
function HorarioModal({
    ...
    const [horario, setHorario] = useState<Record<string, { inicio: string, fin: string } | null>>(() => {
        ...
        diasSemana.forEach(dia => {
            // @ts-ignore
            initial[dia] = profesional.horario_laboral?.[dia] || null
        })
        return initial
    })
    ...
    const [almuerzo, setAlmuerzo] = useState({
        inicio: profesional.bloqueo_almuerzo?.inicio || '14:00',
        fin: profesional.bloqueo_almuerzo?.fin || '15:00',
        activo: !!profesional.bloqueo_almuerzo
    })
            ...
            <div className="glass-card w-full max-w-2xl animate-slide-in max-h-[90vh] overflow-y-auto border border-border">
                <div className="flex items-center justify-between p-6 border-b border-border">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">{getLabel('professional', 'singular', true)}: {profesional.nombre}</p>
                    </div>
                    <button onClick={onClose} className="p-2 rounded-lg hover:bg-surface-hover transition-colors">
                        <svg className="w-5 h-5 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
